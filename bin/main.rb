...
loop do
  ...
  print 'Would you like to export the results of this session?(Enter 1 for Yes, or else (i.e. 0) to skip) '
  exp = gets.chomp.to_i
  if exp == 1
    new_export = Export.new(search_obj)
    new_export.export_data(exp)
  end
  print 'Would you like to perform another search? (Enter 1 for Yes, or else (i.e. 0) to skip) '
  ans = gets.chomp.to_i
